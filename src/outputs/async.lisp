    ...
    (let (entry)
      (with-lock-held ((async-output-lock output))
        ...
        (when (and (async-output-shutdown output)
          ...
          (condition-variable-broadcast (async-output-drained output))
          (return))
        (let ((queue (async-output-queue output))
          ...
          (condition-variable-signal (async-output-not-full output))
    ...
    (with-lock-held ((async-output-lock output))
      ...
      (unless (async-output-shutdown output)
        (let ((queue (async-output-queue output))
          ...
          (condition-variable-signal (async-output-not-empty output))))))
  (values))
...
(defmethod close-output ((output async-output))
  (with-lock-held ((async-output-lock output))
    ...
    (condition-variable-broadcast (async-output-not-empty output))
    (condition-variable-broadcast (async-output-not-full output)))
